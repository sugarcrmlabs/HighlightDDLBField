...
// as uksort is blacklisted, we have to do that ourselves
$keys = array_keys($ACLAllowedModules);
sort($keys);
$ACLAllowedModules = array_combine($keys, array_intersect_key($ACLAllowedModules, array_flip($keys)));
...
$help_img = smarty_function_sugar_help(['text' => $mod_strings['LBL_OPTIONAL_HELP']], $sugar_smarty);
$chart_data_help = smarty_function_sugar_help(['text' => $mod_strings['LBL_CHART_DATA_HELP']], $sugar_smarty);
$do_round_help = smarty_function_sugar_help(['text' => $mod_strings['LBL_DO_ROUND_HELP']], $sugar_smarty);
...
foreach ($ACLAllowedModules as $module => $singular) {
    ...
    $buttons[] = ['name' => $app_list_strings['moduleList'][$module], 'img' => $icon_name, 'key' => $module];
}
...
if ($runQuery == 1) {
    ...
    echo $sugar_smarty->fetch('custom/modules/Reports/ReportsWizard.tpl');
    echo '<br/><br/>';
    echo "<div id='resultsDiv' name='resultsDiv'>";
    //$image_path = $orig_image_path;
    reportResults($args['reporter'], $args);
    echo '</div>';
} elseif ($saveReport !== null && ($saveReport == 'on')) {
    $args = [];
    $report_def = [];
    $report_name = '';
    if (!empty($reportDef)) {
...
} elseif ($isDelete !== null && ($isDelete == '1')) {
    ...
    if (!empty($saveAs)) {
        ...
        if (!empty($saveAsReportType)) {
            ...
            if ($new_report_type == 'summation') {
                ...
                $report_def['display_columns'] = [];
            } elseif ($new_report_type == 'tabular') {
                $report_def['report_type'] = $new_report_type;
                $report_def['group_defs'] = [];
                if (isset($report_def['layout_options'])) {
                    unset($report_def['layout_options']);
                }
                $report_def['summary_columns'] = [];
            } elseif ($new_report_type == 'summation_with_details') {
                if (isset($report_def['layout_options'])) {
                    unset($report_def['layout_options']);
                }
                $report_def['report_type'] = $new_report_type;
            } elseif ($new_report_type == 'matrix') {
                $report_def['report_type'] = 'summary';
...
function setSortByInfo(&$reporter, &$smarty)
{
    ...
    if (!empty($reporter->report_def['summary_order_by'][0]['group_function']) && $reporter->report_def['summary_order_by'][0]['group_function'] == 'count') {
    ...
    } elseif (isset($reporter->report_def['summary_order_by'][0]['name'])) {
        ...
        } elseif (!empty($reporter->report_def['summary_order_by'][0]['column__function'])) {
